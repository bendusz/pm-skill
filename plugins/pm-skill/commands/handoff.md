---
description: End a session cleanly — write pm/HANDOFF.md, a token-efficient briefing so the next agent resumes at full speed.
---

Use the `project-manager` skill to write an end-of-session handoff.

Write `pm/HANDOFF.md` from `${CLAUDE_PLUGIN_ROOT}/templates/HANDOFF.md.template`, **overwriting**
...
- **Commit the `pm/` files** — a handoff that isn't in the repo doesn't survive the session.

`/pm-skill:resume` reads this file (after `pm/pm-state.json`) to skip re-discovery. If state has
moved on since the handoff was written (`pm-state.json.updated` is newer), the resume trusts state +
log over the handoff.
