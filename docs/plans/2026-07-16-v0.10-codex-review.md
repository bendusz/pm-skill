...
- [x] **Step 1: Write the command file** with exactly this content:
...
- [x] **Step 2: Sanity-check the file** — frontmatter parses (single `description:` line), no
...
- [x] **Step 1: SKILL.md note** — after the line
...
- [x] **Step 2: README command row** — after the `/pm-skill:resume` row add:
...
- [x] **Step 3: CHANGELOG entry** — new section at the top (below the header), dated today:
...
- [x] **Step 4: plugin.json** — `"version": "0.9.3"` → `"version": "0.10.0"`.
...
- [x] **Step 2: README** — command row after the codex-review row:
...
- [x] **Step 3: CHANGELOG** — add to the 0.10.0 section:
...
- [x] **Step 4: SKILL.md** — extend the codex note (added in Task 2) with:
...
- [x] **Step 1: Preflight failure path** — `PATH=/usr/bin:/bin command -v codex`; expect no
  output (exit 1), confirming the missing-codex message would trigger.
- [x] **Step 2: Single worktree review** — make a scratch edit (e.g. add a deliberately buggy
...
- [x] **Step 3: Parallel objectives** — scope `worktree` with `security bugs`: two background
  codex agents at once; expect two reports + one index file, both agents exit 0.
- [x] **Step 4: Recent scope** — scope `recent`, no objectives: `codex exec review --commit HEAD …`
  exits 0 with a report.
- [x] **Step 5: codex-help round-trip** — follow codex-help with a small real question about this
  repo (e.g. naming/structure of the new command); expect exit 0 and a substantive answer in the
  scratch file.
- [x] **Step 6: Clean up** — delete the scratch edit; keep `codex/` reports (untracked). Record
